...
# Modify site with customer name
# echo pwd:
# pwd
# cp -rf $PWD/src/ReadingTime3.Web/Views/Home/* $DIR/../
# Add customer name to welcome sign
sed -i.bak -e "s|Customer|$CUSTOMER|g" $PWD/src/ReadingTime3.Web/Views/Home/Index.cshtml
# rm Index.cshtml.bak
...
if [ "$COMMIT_CHANGES" = true ]; then
  git add $PWD/src/ReadingTime3.Web/Views/Home/Index.cshtml
...
if [ "$INTERACTIVE" = false ]; then
  cat <<- EOF > $DIR/issues/code-feature.json
  {
    ...
    In order to create a pull request from the [new branch](https://github.com/${OCTO_ORG}/${OCTO_REPO}/tree/$BRANCH), click [here](https://github.com/${OCTO_ORG}/${OCTO_REPO}/compare/main...${OCTO_ORG}:${BRANCH}?expand=1&title=PR%20for%20customer%20branch%20${BRANCH}).\n
