...
# - resets main branch to refs/tags/baseline
# - closes all open issues
# - creates a new issue describing the development task


# In interactive mode:
# - disables branch protection on main if present
# - resets main branch to refs/tags/baseline
# - enables branch protection on main if it was present
# - closes all open issues
...
if [ "$INTERACTIVE" = true ]; then
  ...
  BRANCHES_STATUS_CODE=$(curl -I -H "Authorization: Token $GITHUB_COM_TOKEN" -H "Accept: application/json" https://api.github.com/repos/${OCTO_ORG}/${OCTO_REPO}/branches/main/protection -s -o /dev/null -w %{http_code})

  if [ $BRANCHES_STATUS_CODE -ne "404" ] ; then

      echo "main is protected: disabling protection"

      # Read required status currently activated on main branch
      CONTEXTS=$(curl -H "Authorization: Token $GITHUB_COM_TOKEN" -H "Accept: application/json" https://api.github.com/repos/${OCTO_ORG}/${OCTO_REPO}/branches/main/protection/required_status_checks/contexts)
      ...
      curl -H "Authorization: Token $GITHUB_COM_TOKEN" -H "Accept: application/json" -H "Content-type: application/json" -X DELETE https://api.github.com/repos/${OCTO_ORG}/${OCTO_REPO}/branches/main/protection
...
echo "Reverting main to baseline tag"
git fetch origin --tags --force
git checkout main
git reset --hard baseline
git push origin baseline:main -f
...
if [ "$INTERACTIVE" = true ]; then
  if [ "$BRANCHES_STATUS_CODE" -ne 404 ] ; then
      ...
      sed -e "s|CONTEXTS_PLACEHOLDER|$CONTEXTS|g" scripts/protected.json | curl -H "Authorization: Token $GITHUB_COM_TOKEN" -H "Accept: application/json" -H "Content-type: application/json" -X PUT https://api.github.com/repos/${OCTO_ORG}/${OCTO_REPO}/branches/main/protection -d @-
  fi
fi

# Comment out issue closing and opening new issue
: '
